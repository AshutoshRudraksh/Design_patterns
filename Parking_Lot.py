...
class ParkingFloor:
  ...
  def park_vehicle(self, vehicle):
    ...
    while r < len(self._spots):
      if self._spots[r] !=0: # check if the spot it empty 
        l = r + 1
      if r - l + 1 == size:
        # if empty and have enough space for the vehicle then park
...
class ParkingSystem:
  ...
  def park_vehicle(self, driver):
    ...
    if isParked:
      self._timeParked[driver.get_vehicle()] = currentHour
    return isParked
  ...
  def remove_vehicle(self, driver):
    if driver.get_id() not in self._timeParked:
      return False
    currentHour = datetime.datetime.now().hour
    timeParked = math.ceil(currentHour - self._timeParked[drive.get_id()])
    driver.charge(timeParked * self._hourRately)
    del self._timeParked[driver.get-id()]
    return self._parkingGarage.remove_vechile(driver.get_vehicle())
